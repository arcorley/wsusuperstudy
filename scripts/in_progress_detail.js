...
$(document).ready(function(){
	...
	if ($("#disinterest").val() == 'no'){
		$("#disinterest_reason_hidden").hide();
	}

	if ($("#ineligible_no_full_screen").val() == 'no'){
...
function updateInProgress(){
	...
	req.open('POST', path + 'update_in_progress', true);
...
function insertToPerm(){
	var check = confirm("Are you sure you want to insert this participant to the permanent dataset?");
	if (check == true)
	{
		var ppt_id = document.getElementById("ppt_id").value;
		var ppt_first_nm = document.getElementById("ppt_first_nm").value;
		...
		var req = new XMLHttpRequest();
		req.open('POST', path + 'insert_to_perm', true);
		req.setRequestHeader("Content-Type", "application/json");
		req.setRequestHeader("X-Forwarded-Proto", "https");
